-- Seed data for Vendors, Device Types (Type-level specs with clean electrical ratings) and Physical Devices (Unit-level specs with network Map)
...
-- Seed Device Types (Type-level specs: dimensions Map, vendor Map, electrical Map without unit suffixes)
INSERT INTO device_types (id, name, sku, archetype_id, dimensions, vendor, electrical) VALUES
  (
    ...
    '{"voltageNominal": 3.6, "voltageMin": 3.0, "voltageMax": 4.2, "currentMax": 120, "powerActive": 432, "powerSleep": 18}'::jsonb
  ),
  (
    ...
    '{"voltageNominal": 48.0, "voltageMin": 12.0, "voltageMax": 54.0, "currentMax": 1250, "powerActive": 15000, "powerSleep": 1200000}'::jsonb
  ),
  (
    ...
    '{"voltageNominal": 6.0, "voltageMin": 4.5, "voltageMax": 7.2, "currentMax": 350, "powerActive": 2100, "powerSleep": 45}'::jsonb
  )
ON CONFLICT (id) DO NOTHING;

-- Seed Physical Inventory Units (Unit-level specs: network Map with MAC/DevEUI/IMEI & deployment powerSource)
INSERT INTO devices (id, device_type_id, serial_number, name, lifecycle_state, network) VALUES
  (
  ...
  (
    ...
    'AVAILABLE',
    '{"powerSource": "POE", "eth": {"macAddress": "00:1B:44:11:3A:08", "speed": 1000, "poeSupported": true}, "wifi": {"macAddress": "00:1B:44:11:3A:09", "supportedStandards": ["802.11n", "802.11ac"], "frequencyBands": ["2.4", "5.0"]}, "gsm": {"imei": "354892019283018", "iccid": "893301928301928301F", "technologies": ["4G", "LTE-M"]}}'::jsonb
  ),
  (
